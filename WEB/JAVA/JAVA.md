...
### 优化正则
-   **将贪婪模式转换为懒惰模式或者独占模式**
    虽然说转为懒惰模式并不会解决回溯问题，但是可以有效的减少回溯的次数
-   **能明确指定范围的，就不要使用统配**
    比如只需匹配`[abc]`的，就不要使用`[a-z]`；只需匹配标点符号的`[\pP\pS\pZ]`，就不要使用通配符`(.?)`
* 减小数量表达式的取值范围
* 不用正则  剔除特殊符号 再查找
